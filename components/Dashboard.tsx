...
// ⬇️ C2.2: datos multi-partido (sin routing)
import { MATCHES, DEFAULT_MATCH } from '../content/matches';
...
const StatCard = ({ label, value, subtext, icon: Icon, color = "emerald" }: any) => (
  <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-start space-x-4 hover:border-slate-700 transition-all">
    {/* Nota: estas clases dinámicas requieren safelist en Tailwind si haces purge estricto */}
...
const MatchOverview = ({ data, analysis }: { data: any, analysis: any }) => {
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800 flex flex-col md:flex-row items-center justify-between shadow-lg relative overflow-hidden">
        ...
        <div className="text-center md:text-right z-10">
          <h1 className="text-3xl font-black text-white tracking-tight uppercase">{data?.meta?.opponent ?? "RIVAL"}</h1>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 lg:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-sm text-slate-300">
                  <span>Asturias</span>
                  <span>{data?.stats?.possession?.[0] ?? 0}%</span>
                </div>

                <div className="flex h-3 rounded-full overflow-hidden bg-slate-800">
                  <div className="bg-emerald-500" style={{ width: `${data?.stats?.possession?.[0] ?? 0}%` }}></div>
                  <div className="bg-blue-500" style={{ width: `${data?.stats?.possession?.[1] ?? 0}%` }}></div>
                </div>

                <div className="flex justify-between text-sm text-slate-300 text-right">
                  <span>{data?.stats?.possession?.[1] ?? 0}%</span>
                  <span>Rival</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div className="bg-slate-950 p-3 rounded-lg text-center">
                  ...
                  <p className="text-xl font-bold text-emerald-400">{data?.stats?.passes?.[0] ?? 0}</p>
                </div>
                <div className="bg-slate-950 p-3 rounded-lg text-center">
            ...
            <div className="space-y-4">
              ...
              <ProgressBar value={data?.stats?.shots?.[0] ?? 0} max={20} label="Tiros Totales" />
              <ProgressBar value={data?.stats?.shotsOnTarget?.[0] ?? 0} max={10} label="Tiros a Puerta" color="bg-blue-500" />
              <ProgressBar value={data?.stats?.duelsWonPercentage?.[0] ?? 0} max={100} label="% Duelos Ganados" color="bg-yellow-500" />
              <ProgressBar value={data?.stats?.corners?.[0] ?? 0} max={10} label="Saques de Esquina" color="bg-purple-500" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          ...
          <div className="space-y-4 text-sm">
            <div className="p-3 bg-slate-950 rounded-lg border-l-2 border-emerald-500">
              ...
              <p className="text-slate-200">{analysis?.teamDNA?.style ?? "—"}</p>
            </div>
            <div className="p-3 bg-slate-950 rounded-lg border-l-2 border-blue-500">
              ...
              <p className="text-slate-200">{analysis?.teamDNA?.defense ?? "—"}</p>
            </div>

            <div>
              <p className="text-slate-400 text-xs uppercase mb-2">Puntos Fuertes</p>
              <div className="flex flex-wrap gap-2">
                {(analysis?.teamDNA?.strengths ?? []).map((s: string, i: number) => (
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {(analysis?.teamDNA?.weaknesses ?? []).map((s: string, i: number) => (
      ...
      {/* Editable Analysis (Texto) */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <SectionHeader title="Análisis (texto)" subtitle="Secciones editables con anclas" />
        <div className="space-y-6">
          {(analysis?.sections ?? []).map((sec: any) => (
            <div key={sec.id ?? sec.title} id={sec.id} className="scroll-mt-24">
              <h3 className="text-lg font-bold text-white">{sec.title}</h3>
              {sec.paragraphs?.map((p: string, i: number) => (
                <p key={i} className="text-slate-300 text-sm leading-relaxed mt-2">{p}</p>
              ))}
            </div>
          ))}
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl relative">
        ...
        {isLocalhost && (
          <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg flex items-start space-x-2">
            ...
            <p className="text-xs text-yellow-200">
              <strong>Nota Importante:</strong> Esta URL funcionará correctamente si la aplicación está desplegada en un entorno accesible.
            </p>
          </div>
        )}
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 flex items-center space-x-2 mb-6">
          ...
          <button
            onClick={handleCopy}
            className={`p-2 rounded-md transition-all ${copied ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
...
export default function Dashboard() {
  ...
  const data = selectedBundle?.data ?? selectedBundle?.match ?? selectedBundle ?? {};
  const analysis = selectedBundle?.analysisContent ?? selectedBundle?.analysis ?? {
    title: 'Análisis',
    updatedAtISO: '',
    teamDNA: { style: '—', defense: '—', strengths: [], weaknesses: [] },
    sections: []
  };

  const p1 = (data?.players?.[0] ?? null) as PlayerStats | null;
  const p2 = (data?.players?.[1] ?? null) as PlayerStats | null;

  useEffect(() => {
    if (typeof window !== "undefined") {
      const params = new URLSearchParams(window.location.search);
      if (params.get('view') === 'public') setIsPublicView(true);
      ...
      // (Opcional) si quieres permitir ?match=KEY sin routing
      const matchParam = params.get('match');
      if (matchParam && (MATCHES as any)?.[matchParam]) {
        setSelectedMatchKey(matchParam);
      }
    }
  }, []);
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      ...
      // Añadimos match al share URL, sin routing
      const newUrl = `${baseUrl}?report_id=AST-EUS-${uniqueId}&view=public&tab=${activeTab}&match=${encodeURIComponent(selectedMatchKey)}`;
  ...
  const handleChangeMatch = (newKey: string) => {
    setSelectedMatchKey(newKey);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col md:flex-row relative">
      ...
      <aside className="w-full md:w-64 bg-slate-900 border-r border-slate-800 flex-shrink-0 md:h-screen sticky top-0 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          <button
            ...
            disabled={!p1}
            className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all ${activeTab === 'p1' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'} ${!p1 ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <div className="flex items-center space-x-3">
              <User size={18} />
              <span>{p1?.name ?? 'Jugador/a 1'}</span>
            </div>
          ...
          <button
            ...
            disabled={!p2}
            className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all ${activeTab === 'p2' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'} ${!p2 ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <div className="flex items-center space-x-3">
              <User size={18} />
              <span>{p2?.name ?? 'Jugador/a 2'}</span>
            </div>
          ...
          <button
            ...
            disabled={!p1 || !p2}
            className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all ${activeTab === 'compare' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'} ${(!p1 || !p2) ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <Users size={18} />
      ...
      <main className="flex-1 h-screen overflow-y-auto bg-slate-950 p-6 md:p-10 relative scroll-smooth">
        ...
        <div className="bg-slate-950">
          ...
          <header className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold text-white flex items-center space-x-2">
                <span>{analysis?.title ?? 'Análisis'}</span>
              </h1>
              <p className="text-slate-500 text-sm mt-1 flex items-center space-x-2">
                <Map size={14} />
                <span>
                  {data?.meta?.competition ?? '—'} • {data?.meta?.date ?? '—'} • Actualizado: {analysis?.updatedAtISO ?? '—'}
                </span>
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 flex items-center gap-2">
                <span className="text-xs text-slate-400">Partido</span>
                <select
                  value={selectedMatchKey}
                  onChange={(e) => handleChangeMatch(e.target.value)}
                  className="bg-slate-950 border border-slate-800 text-slate-200 text-sm rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                >
                  {matchKeys.map((k) => {
                    const b: any = (MATCHES as any)?.[k] ?? {};
                    const d: any = b?.data ?? b?.match ?? b ?? {};
                    const a: any = b?.analysisContent ?? b?.analysis ?? {};
                    const label =
                      a?.title ||
                      (d?.meta?.opponent ? `Asturias vs ${d.meta.opponent}` : k);
          ...
          {activeTab === 'overview' && <MatchOverview data={data} analysis={analysis} />}
          {activeTab === 'p1' && p1 && <PlayerAnalysis player={p1} />}
          {activeTab === 'p2' && p2 && <PlayerAnalysis player={p2} />}
          {activeTab === 'compare' && p1 && p2 && <Comparison p1={p1} p2={p2} />}
        </div>
